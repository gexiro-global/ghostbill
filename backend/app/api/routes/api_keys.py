...
def _key_to_response(key: ApiKey) -> ApiKeyResponse:
    return ApiKeyResponse(
        id=str(key.id), key_prefix=key.key_prefix, label=key.label,
        environment=key.environment, is_active=key.is_active,
...
async def list_api_keys(
    ...
    result = await paginate_cursor(
        db=db, base_query=base_query, model=ApiKey,
...
async def create_api_key(
    ...
    count_stmt = select(func.count(ApiKey.id)).where(
        ApiKey.merchant_id == merchant.id, ApiKey.is_active == True)
    ...
    api_key = ApiKey(
        merchant_id=merchant.id, key_hash=key_hash, key_prefix=key_prefix,
        label=body.label, environment=body.environment, is_active=True,
    )
    db.add(api_key)

    audit = AuditLog(
        merchant_id=merchant.id, action="api_key.created",
    ...
    return ApiKeyCreateResponse(
        id=str(api_key.id), key=plaintext_key, key_prefix=key_prefix,
        label=body.label, environment=body.environment,
    )
...
async def revoke_api_key(
    ...
    api_key = (await db.execute(
        select(ApiKey).where(ApiKey.id == kid, ApiKey.merchant_id == merchant.id)
    )).scalar_one_or_none()
    if api_key is None:
    ...
    audit = AuditLog(
        merchant_id=merchant.id, action="api_key.revoked",
        entity_type="api_key", entity_id=api_key.id,
