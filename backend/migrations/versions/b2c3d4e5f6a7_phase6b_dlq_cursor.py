...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column("delivery_id", UUID(as_uuid=True), sa.ForeignKey("webhook_deliveries.id"), nullable=False),
        ...
        sa.Column("dead_lettered_at", sa.DateTime(timezone=True), server_default=sa.func.now(), nullable=False),
    ...
    for table in ["invoices", "customers", "subscriptions", "webhook_deliveries", "api_keys"]:
        op.create_index(
